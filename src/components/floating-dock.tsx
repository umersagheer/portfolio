...
const FloatingDockMobile = ({
  ...
  return (
    <div className={cn('fixed bottom-5 right-5 block md:hidden', className)}>
      ...
      >
        <SquareStackIcon className='h-5 w-5 text-neutral-500 dark:text-neutral-400' />
      </button>
    </div>
  )
}
  ...
  return (
    <Link href={href}>
      <motion.div
        ...
        onClick={() => {
          title === 'Theme' && setTheme(theme === 'dark' ? 'light' : 'dark')
        }}
      >
        <AnimatePresence>
          {hovered && (
            <motion.div
              ...
              className='absolute -top-8 left-1/2 w-fit -translate-x-1/2 whitespace-pre rounded-large border border-gray-200 bg-content1 px-2.5 py-1 text-xs text-neutral-700 dark:border-neutral-900 dark:bg-content1 dark:text-white'
            >
              {title}
            </motion.div>
          )}
