---
  ...
  image: /images/posts/intro-to-nextjs.avif
...
When one appears and the other disappears, Framer Motion morphs between them. Click the tabs below to see it in action:

<LayoutIdDemo />

Notice how the background indicator doesn't just fade — it *slides* from one tab to the next. That's `layoutId` matching the elements and running FLIP between them.

This pattern is everywhere in polished UIs:
- **Tab indicators** that slide between tabs
- **Navigation highlights** that follow your cursor
- **Cards that expand** into full modals
- **List items** that morph between views
...
Try adding and removing items below:

<AnimatePresenceDemo />

Without `AnimatePresence`, removed items would just vanish. With it, they gracefully shrink and fade away. The `layout` prop on each item ensures the remaining items smoothly reflow into their new positions.
