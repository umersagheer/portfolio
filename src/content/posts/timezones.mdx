...
- **Section 3:** The practical guide (databases, ORMs, frontend, backend)
...
This is one of the most common timezone bugs in JavaScript, and it's baked into the language specification:
...
PostgreSQL's own documentation recommends `timestamptz` for most applications. If your system consistently stores UTC (like most JavaScript backends), `timestamp without time zone` works fine — until someone writes a raw SQL query using `AT TIME ZONE`. That's where things break.

### The AT TIME ZONE Trap

The same SQL keyword in PostgreSQL does **opposite things** depending on the input type.

On `timestamptz`, `AT TIME ZONE 'Asia/Karachi'` converts **to** Pakistan time — exactly what you'd expect.

On `timestamp without time zone`, `AT TIME ZONE 'Asia/Karachi'` assumes the value **is already in** Pakistan time and converts it **to UTC**. The opposite direction.

<ATTimeZoneTrapDemo />

This isn't theoretical. In a production analytics dashboard, orders placed at 5 PM Pakistan time showed up in the 7 AM bucket of the Peak Hours chart. The 10-hour shift (not 5) happened because single `AT TIME ZONE` on a bare timestamp converts in the *wrong direction*: it subtracted 5 hours instead of adding them (12:00 stored → assumes it's PKT → converts to 07:00 UTC).

The fix: **double `AT TIME ZONE`**. The first one labels the bare timestamp as UTC (producing a `timestamptz`), the second one converts to the target timezone.
...
If your app also handles scheduling, recurring events, or serves users who cross timezones — keep reading. The rules change completely.
...
Then March 8 rolls around. Clocks spring forward. EST (UTC-5) becomes EDT (UTC-4). Now `14:00 UTC` is **10:00 AM** local time. The team shows up an hour late.
...
Every major Node.js ORM maps `DateTime` to a timestamp type that is **not timezone-aware** by default. This works fine for simple apps but creates subtle bugs the moment you write raw SQL queries or work with multiple timezones.
...
|---|---|---|
| Railway Virginia (EST) | 5 (or 4 in summer) | 9 |
| AWS Mumbai (IST) | 14 | 9 |
| Your Mac (PKT) | 14 | 9 |
...
### Bug 2: The Date Picker Is Off by One Day

**Symptom:** User selects March 29 in a date picker, but the record shows March 28.

**Cause:** The date picker returns `'2026-03-29T00:00:00'` (no Z suffix). JavaScript parses this as local time. For anyone east of UTC, midnight local time is the previous day in UTC.

**Fix:** Append `Z` to force UTC parsing, or send date-only strings (`'2026-03-29'`).

### Bug 3: It Works Locally but Not in Production

**Symptom:** Tests pass on your Mac but fail on CI/CD. Or the opposite — production is correct but local development shows wrong times.

**Cause:** Your Mac's timezone is `Asia/Karachi` (UTC+5). The CI server is UTC. Code that uses `getHours()` or `new Date('2026-03-29T00:00:00')` (local time parsing) produces different results on each machine.

**Fix:** Set `TZ=UTC` in your local `.env` to match production. Use `getUTCHours()` instead of `getHours()`.

### Bug 4: The AT TIME ZONE Production Bug
...
### Before writing code
- [ ] Classify: is this data about the **past** or the **future**?
- [ ] Past → store UTC. Future → store wall time + IANA timezone name
- [ ] Identify who owns the timezone: the event, the user, or the system?

### In the database layer
- [ ] Use `timestamptz` for past events (or maintain a strict UTC convention)
- [ ] Use `timestamp` + `timezone TEXT` columns for future scheduled wall times
- [ ] Never use single `AT TIME ZONE` on `timestamp without time zone` — always double
- [ ] Ensure PostgreSQL server timezone is UTC (`SHOW timezone;`)

### In the API layer
- [ ] Return UTC from all endpoints (ISO 8601 with Z suffix)
- [ ] Accept date strings from date pickers, not localized timestamps
- [ ] Convert "business date" requests to UTC ranges using the user's timezone
- [ ] Set `TZ=UTC` on your Node.js server

### In the frontend
- [ ] Use `Intl.DateTimeFormat` with `timeZone` option for display
- [ ] Never call `getHours()` / `getDay()` — use UTC methods or Intl
...
### In tests
- [ ] Set `TZ=UTC` in test environment to match production
- [ ] Test with dates near midnight in positive-offset timezones (PKT, IST, JST)
- [ ] Test with dates on DST transition days (March and November in US)
- [ ] Test date-only parsing vs date-time parsing

### When debugging
- [ ] Check: what type is the PostgreSQL column? (`timestamp` vs `timestamptz`)
- [ ] Check: is the raw SQL using single or double `AT TIME ZONE`?
- [ ] Check: is the server's `TZ` environment variable set to UTC?
- [ ] Check: are date strings being parsed with or without a Z suffix?
...
- [PostgreSQL Documentation: Date/Time Types](https://www.postgresql.org/docs/current/datatype-datetime.html) — the authoritative reference on `timestamp` vs `timestamptz` behavior
- [Storing UTC is not a Silver Bullet](https://codeblog.jonskeet.uk/2019/03/27/storing-utc-is-not-a-silver-bullet/) — Jon Skeet's excellent post on why UTC alone fails for future events
- [The Complexities of Timezone Handling in Software](https://www.zainrizvi.io/blog/falsehoods-programmers-believe-about-time-zones/) — falsehoods programmers believe about timezones
- [TC39 Temporal Proposal](https://tc39.es/proposal-temporal/docs/) — the future of date/time handling in JavaScript, with first-class timezone support
- [IANA Time Zone Database](https://www.iana.org/time-zones) — the source of truth used by every OS and programming language
- [Computerphile: The Problem with Time & Timezones](https://www.youtube.com/watch?v=-5wpm-gesOY) — Tom Scott's video that makes DST click in under 10 minutes
